...
export default class InitializationDefaultPlaylistsService {
  ...
  private readonly baseAPIUrl = this.configService.get(
    'BASE_MUSIC_API_URL',
  ) as string;
  ...
  async process(): Promise<void> {
    ...
    await Promise.all(
      defaultPlaylistNames.map(async (name) => {
        ...
        const { data }: { data: PlaylistsResults } = await axios.get(
          `${this.baseAPIUrl}/playlists/tracks`,
          { params },
        );
        ...
        data.results.forEach((result) => {
          ...
          if (!newPlaylists.has(playlistName)) {
            ...
            const tracks = result.tracks.map(
              ({ id, name, artist_name, audio, image }) => {
                return new Track({
                  trackId: id,
                  name,
                  artist: artist_name,
